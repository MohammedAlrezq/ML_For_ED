...
```{r}
library(gridExtra)
sum(table(ED_data$A.shift))
names(ED_data)

plot1<- ggplot(ED_data, aes(x = A.shift ))+
  geom_bar()+
  labs(x = "Patient Arrival per Shift", y= "Patients")+
  plotTheme()


plot2 <- ggplot(ED_data, aes(x = Age.Group))+
  geom_bar() +
  labs(x = "Patient Arrival per Shift", y= "Patients")+
  plotTheme()

plot3 <- ggplot(ED_data, aes(x = T.arrive))+
  geom_bar() +
  labs(x = "Patient Arrival per Shift", y= "Patients")+
  plotTheme()


plot4 <- ggplot(ED_data, aes(x = LOS_binary))+
  geom_bar() +
  labs(x = "Patient Arrival per Shift", y= "Patients")+
  plotTheme()


plot5 <- ggplot(ED_data, aes(x = EDLevel))+
  geom_bar() +
  labs(x = "Patient Arrival per Shift", y= "Patients")+
  plotTheme()
...
# Descriptive stat 
```{r}
install.packages("crosstable") # this package provides summary stat for categorical and numeric 
library(crosstable)

# All observation 
des_data <- crosstable(ED_data)
des_data
write.csv(des_data, "All_Obs.csv")


#less than 4 hours (No = 0)
Less_than_4 <- ED_data |> 
  filter(LOS_binary == "No")
Less_than_4_summary <- crosstable(Less_than_4)
write.csv(Less_than_4_summary, "Less_than_4.csv")

# greater than 4 hours (Yes = 1)
Greater_than_4 <- ED_data |> 
  filter(LOS_binary == "Yes")
Greter_than_4_summary <- crosstable(Greater_than_4)
write.csv(Greter_than_4_summary, "greater_than_4.csv")
```
...
# save the model 
saveRDS(log_model,"C:/Users/alrez/OneDrive - Virginia Tech/Courses_Semesters/Fall 2023/ML ECE/Project/Project_Coding/ML_For_ED/log_model.rds")
```
...
# grid <- expand.grid(
# mtry = c(2, 5, 9 ,10, 12))            # number of variables randomly sampled as candidates at each split
...
#Model 2: Random forest
set.seed(123400)
rf_model <- train(LOS_binary~.-A.Hr -W.FP -Pat_count -Month -A.Day -FirstWard,
                    data = train,
                    method = "rf",
                    metric = "ROC",
                    trControl = control,
...
# Save the model to a specified path with the .rds extension
saveRDS(rf_model, "C:/Users/alrez/OneDrive - Virginia Tech/Courses_Semesters/Fall 2023/ML ECE/Project/Project_Coding/ML_For_ED/rf_model.rds")

# Load the model from the .rds file
#rf_model <- readRDS("C:/Users/alrez/OneDrive - Virginia Tech/Courses_Semesters/Fall 2023/ML ECE/Project/Project_Coding/ML_For_ED/rf_model.rds")

# Assuming you have new data called new_data, you would run the model like this:
# Make sure that new_data has the same structure (same variables) as the data used to train the model
...
grid <- expand.grid(
  nrounds = c(100, 150, 200),  # Including the best tune value and exploring around it
  max_depth = c(2, 3, 4),      # One below and one above the best tune value
  eta = c(0.1, 0.3, 0.5),      # Exploring lower and higher values
  gamma = c(0, 0.1, 0.2),      # Exploring slightly higher values
  colsample_bytree = c(0.6, 0.8, 1.0), # Exploring a range around the best tune value 
  min_child_weight = c(1, 2, 3), # Exploring a range around the best tune value
  subsample = c(0.6, 0.8, 1)   # Exploring a range around the best tune value
)
# Xgboost
set.seed(123411)
...
# Tune the model 
modelLookup("xgbTree")# to chekc which parameters associated with the algorithm
xgboost_model$bestTune # to check which parameters associated with the model 

plot(xgboost_model$bestTune$nrounds)
...
# Model 4: NN 
```{r}
tuneGrid_NN <- expand.grid(size = c(1, 2, 3, 4, 5, 6),
